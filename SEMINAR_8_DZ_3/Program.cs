...
{
    System.Console.WriteLine($"Перемножить данные матрицы невозможно:" +
    $" число столбцов первой ({array1.GetLength(1)}) не равняется" +
    $" числу строк второй ({array2.GetLength(0)})");
}
else
{
    System.Console.WriteLine("Результат - произведение матриц:");
    PrintArrayMatrix(MatrixProduct(array1, array2));
}
